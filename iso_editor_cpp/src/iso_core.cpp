...
bool ISOCore::loadCueSheet(const QString &filePath) {
    try {
        ...
        qCritical() << "Caught exception while loading CUE sheet:" << e.what();
        return false;
    }
}
...
bool ISOCore::loadIso(const QString &filePath) {
    try {
        ...
        qCritical() << "Caught exception while loading ISO:" << e.what();
        return false;
    }
}
...
QByteArray ISOCore::getFileData(const IsoNode* node) const {
    try {
        ...
        qCritical() << "Caught exception in getFileData:" << e.what();
        return QByteArray();
    }
}
...
bool ISOCore::saveIso(const QString &filePath, bool useUdf, bool makeHybrid) {
    try {
        ...
        qCritical() << "Caught exception while saving ISO:" << e.what();
        return false;
    }
}
...
static void buildNodeTree(ISO9660::IFS &fs, const std::string& path, IsoNode* parent_node) {
    try {
        ...
        qCritical() << "Caught exception in buildNodeTree:" << e.what();
    }
}
...
static bool writeTreeToDisk(const ISOCore* core, const IsoNode* node, const QString& currentPath) {
    try {
        ...
        qCritical() << "Caught exception in writeTreeToDisk:" << e.what();
        return false;
    }
}
